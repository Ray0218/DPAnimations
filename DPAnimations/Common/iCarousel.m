...
@synthesize offsetMultiplier; //偏移比例
...
- (CATransform3D)transformForItemView:(UIView *)view withOffset:(CGFloat)offset
{
    ...
    CGFloat tilt = 0.9f;
    CGFloat spacing =0.5 ;// 0.25f; // should be ~ 1/scrollSpeed; //调整小图之间的间距
    ...
    CGFloat x = (clampedOffset * 0.5f * tilt + offset * spacing) * itemWidth; //调整大图与小图的间距
    CGFloat z = fabsf(clampedOffset) * -itemWidth * 0.5f;
   
    transform = CATransform3DTranslate(transform, x*1.4, 0.0f, 3*z );
    
    //修改Y轴的旋转
    return CATransform3DRotate(transform, -clampedOffset * M_PI_2 * tilt, 0.0f, 0.0f, 0.0f);
    //perform transform
}
